...
import { posterData } from '../data/poster.content.data';

@Injectable()
export class LangchainChatService {
  logger = new Logger(LangchainChatService.name);
  ...
  async uploadJSON(documentDto?: JsonDocumentDto) {
    this.logger.log(`Uploading JSON data: ${JSON.stringify(documentDto)}`);
    try {
      const jsonData = JSON.stringify(posterData);
      ...
      // Convert JSON data to a string if it is not already
      const jsonString =
        typeof jsonData === 'string' ? jsonData : JSON.stringify(jsonData);

      // Split the JSON string into manageable chunks
      const splitTexts = await textSplitter.splitText(jsonString);
      const embeddings: Document[] = [];
